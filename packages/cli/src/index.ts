...
function formatCheckLine(check: { ok: boolean; id: string; message: string }): string {
...
async function runExplainCommand(args: string[]): Promise<number> {
  ...
  try {
    const envelope = await loadJsonFile<SpurEnvelope>(envelopePath);
    const policy = await loadJsonFile<GovernancePolicy>(policyPath);
    const robot = await loadJsonFile<RobotProfile>(robotPath);

    let adapter: AdapterContract | undefined = getAdapterById(adapterArg);
    if (!adapter) {
      adapter = await loadJsonFile<AdapterContract>(adapterArg);
    }
